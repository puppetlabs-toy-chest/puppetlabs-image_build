...
describe Puppet::Face[:aci, '0.1.0'] do
  ...
  %i[script build].each do |subcommand|
    describe "##{subcommand}" do
      ...
      it 'should fail if not passed a manifest because default manifest does not exist' do
        expect(PuppetX::Puppetlabs::AciBuilder).to receive(:new).with('manifests/init.pp', any_args).and_call_original
      ...
      it 'should fail if passed non existent manifest' do
        expect(PuppetX::Puppetlabs::AciBuilder).to receive(:new).with('not-a-real-file', any_args).and_call_original
  ...
  describe '#build specific options' do
    ...
    it 'should catch issues with the underlying build' do
      expect_any_instance_of(PuppetX::Puppetlabs::AciBuilder).to receive(:build).and_raise(PuppetX::Puppetlabs::BuildError)
    ...
    it 'should not fail if passed a master even when default manifest does not exist' do
      expect(PuppetX::Puppetlabs::AciBuilder).to receive(:new).with('manifests/init.pp', any_args).and_call_original
      expect_any_instance_of(PuppetX::Puppetlabs::AciBuilder).to receive(:build)
